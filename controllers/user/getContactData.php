...
if($search_email!="")
{
    ...
    $filtersArr["middle_name"]=array("LIKE","%$search_email%");
...
if($search_phone!="")
{
    ...
    $filtersArr["landline_number"]=array("LIKE","%$search_phone%");
}
...
<table class="table table-bordered table-hover ">
    ...
    <tr>
        ...
        <th style="background-color: lightblue;width:20%">Note</th>
        ...
        <?php } else {
            foreach ($results as $value) { ?>
                <tr>
                    <td>
                        <img src="<?= $rootUrlImages.$value['photo_path'] ?>" alt="Contact Photo" height="100" width="100">
                    </td>
                    <td>
                    ...
                    <td>
                        <?php echo "Mobile : ".$value['moblie_number']."<br/>"; ?>
                        <?php echo "Landline : ".$value['landline_number']; ?>
                    </td>
                    <td>
                        <?php echo $value['notes'] ?>
                    </td>
                    <td class="text-center">
                        <a href="#" class="btn btn-primary btn-xs" data-toggle="modal" data-target="#editContactModal" onclick="updateContact('<?= $value['id']; ?>','<?= $value['first_name']; ?>','<?= $value['middle_name']; ?>','<?= $value['last_name']; ?>','<?= $value['moblie_number']; ?>','<?= $value['landline_number']; ?>','<?= $value['email_address']; ?>','<?= $value['notes']; ?>')">Edit</a>
                        <a href="#" class="btn btn-primary btn-xs" onclick="deleteContact('<?= $value['id']; ?>')">Delete</a>
                    </td>
...
<script>
    ...
    function updateContact(id,f_name,m_name,l_name,moblie_number,landline_number,email_address,notes)
    {
        ...
        $("#edit_landline_number").val(landline_number);
